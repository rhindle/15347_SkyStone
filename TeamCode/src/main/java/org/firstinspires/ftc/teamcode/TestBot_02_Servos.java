...
//                .addData("Distance (cm)", "%.01f", robot.sensorDistance.getDistance(DistanceUnit.CM));
    ...
    private void cycleServos () {
        if (servoTime.milliseconds() > 1000) {
            double servoPosition = 0.5;
            servoTime.reset();
            servoState++;
            if (servoState > 25) servoState = 0;
            switch (servoState) {
                case 0:
                    servoPosition = 0.5;
                    break;
            ...
            robot.servo0.setPosition(servoPosition);
            robot.servo1.setPosition(servoPosition);
            robot.servo2.setPosition(servoPosition);
            robot.servo3.setPosition(servoPosition);
            robot.servo4.setPosition(servoPosition);
            robot.servo5.setPosition(servoPosition);
        }
    }
