...
public class Emmet_auto01__LK extends LinearOpMode {
    ...
    private double autoSkystoneY;  // LK double from int
    private boolean flagIsGobilda = false;   // <<========  IMPORTANT
...
// LK    private ElapsedTime autoSkystoneTimer = new ElapsedTime(ElapsedTime.Resolution.SECONDS);
    private final int autoJibPositionGrab = 1300;
    ...
    public void runOpMode() {
        ...
        if (opModeIsActive()) {
            ...
            /* LK */
            if (autoSide == 1) {
                //foundation build side
                autoRoutineFoundation();
            } else if (autoSide == 2) {
                //skystone side
                autoRoutineSkystone();
            }
...
//        vuforiaSkyStone.close();
    ...
    private void initialize() {
        ...
        while (!isStarted()) {
            ...
            telemetry.addData("/////////////////////////////////////////////////////", " ");
            telemetry.addData("location", autoSide == 1 ? "FOUNDATION (BUILD)" : "STONES (QUARRY)");
            telemetry.addData("alliance", autoAlliance == 1 ? "BLUE" : "RED");
            telemetry.addData("parking", autoParkingPosition == 1 ? "NEAR" : "FAR");

            /* LK */
            telemetry.addData("/////////////////////////////////////////////////////", " ");
            telemetry.addData("homed?", flagCraneIsHomed ? "yes" : "NOT HOMED");
...
//        }
    ...
    private void autoReadyGrabber() {
        motorJib.setPower(0);
        //motorJib.setTargetPosition(jibPositionGrab);
        motorJib.setTargetPosition(autoJibPositionGrab);  //LK
        motorJib.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        motorJib.setPower(0.5);
        autoOpenGrabber();
    }
    ...
    /* LK */
    private void autoRoutineFoundation() {
        //no need for vuforia for this routine
        vuforiaSkyStone.deactivate();
        vuforiaSkyStone.close();
    ...
    private void autoRoutineSkystone() {
        autoFlagWhiskers = 0;
        autoFlagGrab = 0;
        autoStowWhiskers();
    ...
    private void lookForSkystones() {
        ElapsedTime autoSkystoneTimer = new ElapsedTime(ElapsedTime.Resolution.SECONDS);
        autoSkystonePattern = 0;
        autoSkystoneY = 0;
        while (opModeIsActive() && autoSkystoneTimer.seconds() < 2) {
            ...
            if (isTargetVisible("Stone Target")) {
                ...
                if (autoAlliance == 1) {
                    ...
                    // red
                    if (autoSkystoneY > -8 && autoSkystoneY < 0) {
                        autoSkystonePattern = 2;
                    } else if (autoSkystoneY > 0) {
                        autoSkystonePattern = 1;
                    } else {
                        autoSkystonePattern = 3;
                    }
